...
def mcp_config(profile: dict, helper_path: str) -> dict:
    return {'mcpServers': {PLUGIN_NAME: {'type': 'http', 'url': profile['mcp_url'],
            'http_headers_helper': shlex.join(['python3', helper_path, identity(profile)]), 'tool_timeout_sec': 300}}}


def snapshot_files(profile: dict, helper_path: str | None = None) -> dict[str, str]:
    profile = validate_profile(profile)
    helper_path = helper_path or str(Path(__file__).resolve().parent / 'auth_headers.py')
    if not Path(helper_path).is_absolute() or Path(helper_path).name != 'auth_headers.py':
        raise ValueError('Credential helper must be an absolute plugin helper path.')
    selected = (
    ...
    files = {
        ...
        'runtime.json': json.dumps({'auth_helper': helper_path}, indent=2) + '\n',
        '.mcp.json': json.dumps(mcp_config(profile, helper_path), indent=2) + '\n',
    }
    for name in ('intake', 'intake-prepare', 'intake-submit'):
        files[f'skills/{name}/references/ACTIVE_CONNECTION.md'] = selected
    return files


def package_profile(package: Path) -> dict:
    profile = validate_profile(json_object((package / 'connection.json').read_text()))
    runtime = json_object((package / 'runtime.json').read_text())
    for relative, expected in snapshot_files(profile, runtime['auth_helper']).items():
...
def saved_token(package: Path, expected_connection: str | None = None) -> str:
    profile = package_profile(package)
    if expected_connection is not None and expected_connection != identity(profile):
        raise ValueError('Credential helper connection binding mismatch.')
    if load_state()['active'] != profile:
        raise ValueError('Package does not match the active connection; run configure.py use from the personal source and reconnect.')
    try:
