...
2. Start a durable session with `complete_intake_details`: include `title`,
   ...
   Ask explicitly for missing purpose, originating environment, AI and human
   contributions, evidence context, limitations/conflicts and sensitivity. An
   unknown origin can be declared unknown by the user; never invent an upstream
   history or fill these human declarations from plausible assumptions.
